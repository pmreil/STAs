class CompanySectorController < ApplicationController
  ...
  def index
    @allSectorViews = SecurityView.select("company_sectors.name as name, count(security_views.id)").joins(:company_sector,:security).includes(:security,:company_sector).group("securities.company_sector_id").order("count(security_id) desc").limit(10)
