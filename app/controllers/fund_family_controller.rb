class FundFamilyController < ApplicationController
  ...
  def show
    if params[:id].nil?
      ...
      @todaysFamilyViews = SecurityView.where("securities.fund_family_id = ? and security_views.created_at >= ?",params[:id],Time.now-1.days).joins(:security).group(:security_id).order("count(security_id) desc").limit(10)
      @thisWeeksFamilyViews = SecurityView.where("securities.fund_family_id = ? and security_views.created_at >= ?",params[:id],Time.now-7.days).joins(:security).group(:security_id).order("count(security_id) desc").limit(10)
      if @family.nil?
